...
use libp2p::swarm::behaviour::{
    AddressChange, ConnectionClosed, ConnectionEstablished, DialFailure, FromSwarm, ListenFailure,
};
use libp2p::swarm::{
    ConnectionDenied, ConnectionHandler, ConnectionHandlerSelect, ConnectionId, NetworkBehaviour,
        ...
        match event {
            ...
            FromSwarm::AddressChange(
                ...
                self.details.entry(peer_id).and_modify(|details| {
                    if let Some(conn) = details.connections.iter_mut().find(|conn| conn == &old) {
                        *conn = new.clone()
                    }
                });
            }
            ...
            match self.identify.poll(cx) {
                ...
                Poll::Ready(ToSwarm::GenerateEvent(event)) => match event {
                    ...
                    IdentifyEvent::Error { peer_id, error, .. } => {
                        log::debug!("Identification with peer={peer_id:?} error={error}")
                    }
