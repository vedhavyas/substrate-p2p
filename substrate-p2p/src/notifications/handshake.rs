    ...
    TSubstream: AsyncRead + AsyncWrite + Unpin + Send + 'static,
{
    ...
    fn upgrade_inbound(self, mut socket: TSubstream, negotiated_name: Self::Info) -> Self::Future {
        log::debug!(
            "HandshakeInbound name={negotiated_name:?} current_name={:?}",
            self.name
        );

        Box::pin(async move {
            let handshake_len = unsigned_varint::aio::read_usize(&mut socket).await?;
            log::trace!("HandshakeInbound length={handshake_len:?} name={negotiated_name:?}",);
            ...
            log::trace!(
                "HandshakeInbound received handshake={handshake:?} name={negotiated_name:?}",
            );
            ...
            match state {
                HandshakeInboundSubstreamState::Sending(handshake) => {
                    match Sink::poll_ready(this.socket.as_mut(), cx) {
                        Poll::Ready(_) => {
                            log::trace!(
                                "HandshakeInboundSubstream: poll_process: Sink is ready start sending name={:?}",
                            ...
                            *this.state = HandshakeInboundSubstreamState::Flush;
                            ...
                            match Sink::start_send(this.socket.as_mut(), io::Cursor::new(handshake))
                            {
                                ...
                                Err(err) => {
                                    log::error!(
                                        "HandshakeInboundSubstream: poll_process: Failed to start sending name={:?} error={:?}",
                                        this.negotiated_name,
                                        err
                                    );
    ...
    TSubstream: AsyncRead + AsyncWrite + Unpin,
{
    ...
    fn poll_next(self: Pin<&mut Self>, cx: &mut Context) -> Poll<Option<Self::Item>> {
        ...
        loop {
            ...
            log::trace!(
                "HandshakeInboundSubstream: poll_next: state={state:?} name={:?}",
                this.negotiated_name
            );
            ...
            match state {
                ...
                HandshakeInboundSubstreamState::NeedsClose => {
                    match Sink::poll_close(this.socket.as_mut(), cx)? {
                        Poll::Ready(()) => {
                            log::trace!(
                                "HandshakeInboundSubstream: poll_close: fully closed name={:?}",
                                this.negotiated_name
                            );
    ...
    TSubstream: AsyncRead + AsyncWrite + Unpin + Send + 'static,
{
    ...
    fn upgrade_outbound(self, mut socket: TSubstream, negotiated_name: Self::Info) -> Self::Future {
        log::debug!(
            "HandshakeOutbound name={negotiated_name:?} current_name={:?}",
            self.name
        );

        Box::pin(async move {
            log::trace!(
                "HandshakeOutbound prepare to write handshake={:?} name={negotiated_name:?}",
                self.handshake,
            );

            write_length_prefixed(&mut socket, &self.handshake).await?;

            log::trace!(
                "HandshakeOutbound prepare to read handshake length name={negotiated_name:?}"
            );

            let handshake_len = unsigned_varint::aio::read_usize(&mut socket).await?;

            log::trace!(
                "HandshakeOutbound handshake len={handshake_len:?} name={negotiated_name:?}",
            );
